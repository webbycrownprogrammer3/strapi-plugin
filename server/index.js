      ...
      if (regex && regex.trim()) {
        try {
          ...
          if (!regexPattern.test(stringValue)) {
            return (
              customErrorMessage || "Value does not match the required pattern"
            );
          }
        } catch (e) {
    ...
    validate: (value, { required, options = {} }) => {
      ...
      if (checkboxType === "multiple") {
        if (minChoices > 0 && values.length < minChoices) {
          return (
            customErrorMessage ||
            `Please select at least ${minChoices} option${minChoices > 1 ? "s" : ""}`
          );
        }
        if (maxChoices > 0 && values.length > maxChoices) {
          return (
            customErrorMessage ||
            `Please select at most ${maxChoices} option${maxChoices > 1 ? "s" : ""}`
          );
        }
      }
...
module.exports = {
  ...
  bootstrap({ strapi }) {
    ...
    setTimeout(() => {
      ...
      if (
        strapi.customFields &&
        typeof strapi.customFields.get === "function"
      ) {
        customFields.forEach((field) => {
          try {
